...
from .evaluation import run_dataset
...
async def evaluation_run() -> dict:
    candidates = [Path("/datasets/evaluation.json"), Path.cwd().parent.parent / "datasets" / "evaluation.json"]
    path = next((candidate for candidate in candidates if candidate.exists()), None)
    if path is None:
        raise HTTPException(status_code=404, detail="EVALUATION_DATASET_UNAVAILABLE")
    return run_dataset(path)
