...
async function seedCommerce() {
  ...
  for (let index = 1; index <= 300; index += 1) {
    ...
    const productId = `prd_${String(((index - 1) % 500) + 1).padStart(4, "0")}`;
...
async function upsertKnowledge({ id, type, visibility, categoryId, authority, title, summary, content, version = "1.0.0", effectiveFrom = "2026-01-01", effectiveTo = null }) {
  ...
  await prisma.knowledgeChunk.upsert({
    where: { id: chunkId }, update: { section: title, content, tokenCount: content.split(/\s+/).length },
    create: { id: chunkId, versionId, section: title, content, tokenCount: content.split(/\s+/).length },
  });
...
async function replaceSyntheticKnowledge() {
  ...
  for (const [categoryId] of categories) {
    ...
    for (const subject of subjects) {
      for (const [state, stateText, action] of states.slice(0, 4)) {
        documentIndex += 1;
