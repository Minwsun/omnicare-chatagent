...
export default async function AiRunDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const run = await prisma.aiRun.findUnique({ where: { id }, include: {
    conversation: { include: { customer: true, messages: { orderBy: { createdAt: "asc" } }, tickets: true } },
    steps: true,
  ...
  return <><Link className="back-link" href="/admin/ai-runs">← AI Runs</Link><p className="eyebrow">AGENT TRACE</p><h1>{run.intent ?? "UNCLASSIFIED"}</h1><p>{run.id}</p>
    ...
    <section className="admin-section"><h2>Ngữ cảnh</h2><article className="data-card"><p>Conversation: {run.conversation.tickets[0] ? <Link href={`/admin/tickets/${run.conversation.tickets[0].id}`}>{run.conversationId}</Link> : run.conversationId}</p><small>{run.conversation.customer?.name ?? "Khách vãng lai"} · prompt {run.promptVersion} · {run.startedAt.toLocaleString("vi-VN")}</small></article></section>
