...
export default function InboxClient() {
  ...
  useEffect(()=>{
    ...
    let active=true;
    void (async()=>{
      let ticket=await loadDetail(selectedId);
      if(!active||!ticket||ticket.assignedTo)return;
      const claimed=await fetch(`/api/admin/tickets/${selectedId}/claim`,{method:"POST"});
      if(claimed.ok&&active)ticket=await loadDetail(selectedId);
    })();
    return()=>{active=false;};
  ...
  return <div className="inbox-layout">
    ...
    <section className="ticket-workspace">{detail?<><header className="ticket-header"><div><small>{detail.id} · {detail.category}</small><h2>{detail.customer?.name as string||"Khách hàng"}</h2><p>{detail.summary}</p></div><div className="ticket-actions">{mine&&<><button disabled={busy} onClick={()=>action("status",{action:"WAIT_CUSTOMER"})}>Chờ khách</button><button disabled={busy} onClick={()=>action("status",{action:"RESOLVE"})}>Đã xử lý</button><button disabled={busy} onClick={()=>action("status",{action:"RELEASE_AI"})}>Trả lại AI</button></>}</div></header>
