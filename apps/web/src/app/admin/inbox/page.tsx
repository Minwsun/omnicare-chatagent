...
export default async function InboxPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [tickets,total] = await Promise.all([prisma.ticket.findMany({ where, select: { id:true,status:true,priority:true,category:true,summary:true,orderId:true,customer:{select:{name:true}},conversation:{select:{_count:{select:{messages:true}}}},_count:{select:{events:true}} }, orderBy: [{ updatedAt: "desc" }], skip:(page-1)*25, take:25 }), prisma.ticket.count({where})]);
  ...
  return <><p className="eyebrow">HUMAN QUEUE</p><div className="admin-title"><div><h1>Yêu cầu hỗ trợ</h1><p>{tickets.length} ticket gần nhất, mới cập nhật trước.</p></div></div>
    ...
    {!tickets.length && <p className="empty-state">Không tìm thấy ticket phù hợp.</p>}<nav className="pagination">{page>1&&<Link href={`?page=${page-1}&search=${encodeURIComponent(search)}&status=${status}&priority=${priority}`}>← Trang trước</Link>}<span>Trang {page}/{Math.max(1,Math.ceil(total/25))}</span>{page*25<total&&<Link href={`?page=${page+1}&search=${encodeURIComponent(search)}&status=${status}&priority=${priority}`}>Trang sau →</Link>}</nav>
  </>;
}
